...
def generate_buy_queue(
    ...
    Self-limits total notional to config.EFFECTIVE_GLOBAL_NOTIONAL_BUDGET_PCT
    (M48: config.GLOBAL_NOTIONAL_BUDGET_PCT unless overridden per-account)
    of portfolio value, same as the run-level budget bot.py enforces as a
    ...
    # M48: EFFECTIVE_* (not the bare constants) -- resolves to the base
    # value unless config.SMALL_ACCOUNT_CONCENTRATED_MODE is set for this
    # process, in which case a small account gets a deliberately
    # concentrated sizing instead of a diversification math that can
    # never clear MIN_ORDER_NOTIONAL at its actual equity. See config.py's
    # own comment on these three names for the full reasoning.
    run_notional_budget = portfolio_value * config.EFFECTIVE_GLOBAL_NOTIONAL_BUDGET_PCT
    deployable_cash = min(deployable_cash, run_notional_budget)
    target_value = portfolio_value / config.EFFECTIVE_TARGET_POSITION_COUNT
    max_value = portfolio_value * config.EFFECTIVE_MAX_SINGLE_POSITION_WEIGHT
    ...
    for _, row in buyable.iterrows():
        if deployable_cash < config.MIN_ORDER_NOTIONAL:
            break
        if position_count >= config.EFFECTIVE_TARGET_POSITION_COUNT:
            break
