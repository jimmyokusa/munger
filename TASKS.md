...
| **Batch-scale live test** of `fetch_all_metrics()` against real concurrency | done | 2026-07-21 — pm-reviewer + staff-engineer-reviewer both flagged that only single-ticker live checks had been run, leaving yfinance's behavior under the actual 12-worker pool unvalidated. Ran against 400 real S&P 400/600 tickers (Wikipedia-sourced, no FMP key needed): **400/400 succeeded, 0 failures, 17.5s elapsed** — no rate-limiting, no hangs observed at this scale. Spot-checked a 10-ticker sample for data sanity (varied real P/E, D/E, ROE, consecutive-years values; a couple of legitimate `None` fields matching the JPM pattern below, e.g. `AAL`'s `debt_to_equity`). Only covers the Wikipedia-sourced 400/600 slice (~27% of the eventual ~1500-ticker composite) — full-scale validation including the FMP-sourced 500 is still gated on M1's blocked live full-composite validation row. |
...
| Tune thresholds against live data (~20–50 buyable names) | todo | if the buyable count lands outside this band, treat it as a signal a threshold needs adjusting, not just noise — record the actual count and the resulting adjustment here; run the sanity-reviewer subagent against each tuning pass. **Explicitly decide the `EXCLUDED_SECTORS`/Financials question flagged in M2**: yfinance commonly lacks `marketCap`/`debtToEquity` for financial-sector tickers (confirmed live for JPM and others), so with the current `EXCLUDED_SECTORS = ()` default those names will fail Graham gates on missing data rather than substantive criteria — decide and record whether that's acceptable or whether Financials should be excluded by default, don't let it pass unnoticed during tuning. |
