...
| **Batch-scale live test** of `fetch_all_metrics()` against real concurrency | done | 2026-07-21 — pm-reviewer + staff-engineer-reviewer both flagged that only single-ticker live checks had been run, leaving yfinance's behavior under the actual 12-worker pool unvalidated. Ran against 400 real S&P 400/600 tickers: **400/400 succeeded, 0 failures, 17.5s elapsed** — no rate-limiting, no hangs observed at this scale. Spot-checked a 10-ticker sample for data sanity (varied real P/E, D/E, ROE, consecutive-years values; a couple of legitimate `None` fields matching the JPM pattern below, e.g. `AAL`'s `debt_to_equity`). Only covers ~27% of the eventual ~1500-ticker composite — a true full-scale run hasn't been done yet. |
