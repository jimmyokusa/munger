...
| **New (staff-engineer-reviewer, M10 review):** lock against overlapping/concurrent `bot.run()` invocations | todo | added 2026-07-22 — nothing today prevents two instances (an overrun run plus its next scheduled trigger, or a manual re-run) from running at once; both would independently read/write `state.json`'s strike counters and `journal.db` from a same-moment holdings snapshot. `client_order_id`'s determinism prevents double-*execution* of the same order (given a shared `run_date`), but does nothing for the state-file race. A scheduling-layer concern (pidfile/lock check before invoking `bot.py`), not something `bot.py` itself should own. |
| **New (staff-engineer-reviewer, M10 review):** per-run abort paths (kill switch, fetch-fraction, both budgets) are indistinguishable from a full success by exit code alone | todo | added 2026-07-22 — every abort in `bot.run()` just logs and returns normally (exit code 0), so a scheduler/alerting setup watching only exit codes can't tell "ran clean and traded" from "aborted, needs attention" without parsing `munger.log`. Feeds directly into the "Alert on run failure" row below — that alerting design needs to treat these aborts as failure-adjacent signals, not just structured log lines. |
| **New (staff-engineer-reviewer, M10 review):** `GLOBAL_ORDER_BUDGET`/`GLOBAL_NOTIONAL_BUDGET_PCT` bound a single run correctly but have no cross-run/cumulative counterpart | todo | added 2026-07-22 — each invocation gets its own fresh budget allowance; combined with the concurrency-lock gap above, nothing bounds total damage across a day if the scheduler fires more than once. |
| Alert on run failure | todo | pm-reviewer noted (2026-07-22, M10 delta review): build this *after* the exit-code-semantics row above, not before or alongside it — an alerting design built against today's exit codes can't distinguish "ran clean and traded" from "aborted, needs attention," since every abort path currently just returns normally. Verify by forcing a failure, not just code review. |
